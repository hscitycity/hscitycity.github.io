...
> 📌 **이 장의 그림은 모두 팔란티어 공식 문서(Architecture Center)의 원본 다이어그램이다.** 각 그림 아래에 출처 문서를 링크해 두었다. 인용·분석 목적이며, 저작권은 Palantir Technologies에 있다.
...
![AIP, Foundry, Apollo와 팔란티어 제품의 관계](https://www.palantir.com/docs/resources/foundry/architecture-center/overview-platforms-and-products.png)
*출처: [Palantir Architecture Center — Overview](https://www.palantir.com/docs/foundry/architecture-center/overview/)*
...
![온톨로지를 층으로 표현한 그림. 위에는 애플리케이션·자동화·에이전트, 아래에는 거버넌스·데이터/로직/워크플로 서비스](https://www.palantir.com/docs/resources/foundry/architecture-center/overview-ontology-diagram.png)
*출처: [Palantir Architecture Center — Overview](https://www.palantir.com/docs/foundry/architecture-center/overview/)*
...
위 그림에서 그 구조가 그대로 보인다. 온톨로지가 가운데 한 층으로 있고, **그 위에 애플리케이션·자동화·에이전트가 나란히 올라간다.** 사람이 쓰는 앱과 AI 에이전트가 같은 층에서 같은 온톨로지를 내려다본다. 아래에는 거버넌스와 데이터·로직·워크플로 서비스가 받친다.

역량을 9개 격자로 정리한 그림도 있다.

![팔란티어 서비스·역량 9개 격자](https://www.palantir.com/docs/resources/foundry/architecture-center/overview-nine-grid.png)
*출처: [Palantir Architecture Center — Overview](https://www.palantir.com/docs/foundry/architecture-center/overview/)*

가운데 줄이 **Ontology Language / Engine / Toolchain**이다. 온톨로지가 격자의 정중앙을 차지한다 — 문서 구조 자체가 "온톨로지가 중심"이라는 주장을 반복하고 있다.
...
![Foundry, AIP, Apollo가 Enterprise Operating System을 구성한다](https://www.palantir.com/docs/resources/foundry/architecture-center/platforms-enterprise-os.png)
*출처: [Palantir Architecture Center — AIP, Foundry, and Apollo](https://www.palantir.com/docs/foundry/architecture-center/platforms/)*
...
![온톨로지가 데이터 소스·로직 소스·시스템 및 액션을 분석/워크플로·자동화·제품/SDK에 연결하는 다이어그램](https://www.palantir.com/docs/resources/foundry/architecture-center/ontology-system.png)
*출처: [Palantir Architecture Center — The Ontology System](https://www.palantir.com/docs/foundry/architecture-center/ontology-system/)*

왼쪽에서 온갖 소스가 들어오고 오른쪽으로 분석·자동화·제품이 나간다. **온톨로지는 그 사이에 낀 번역기**다.

네 가지가 통합된다.

![온톨로지가 보안 계층 위에 있고, 그 아래에 데이터·로직·액션이 있는 구조](https://www.palantir.com/docs/resources/foundry/architecture-center/ontology-system-zoom.png)
*출처: [Palantir Architecture Center — The Ontology System](https://www.palantir.com/docs/foundry/architecture-center/ontology-system/)*

이 그림에서 **보안(Security)이 별도 항목이 아니라 데이터·로직·액션 전체를 가로지르는 층**으로 그려진 게 중요하다. 보안은 나중에 얹는 기능이 아니라 밑에 깔리는 바닥이다.
...
이 세 구성과 앞의 4중 통합을 하나의 표로 겹쳐놓은 그림이 있다. **행이 Language/Engine/Toolchain, 열이 Data/Logic/Action/Security다.**

![행이 Language·Engine·Toolchain, 열이 Data·Logic·Action·Security인 표](https://www.palantir.com/docs/resources/foundry/architecture-center/ontology-table.png)
*출처: [Palantir Architecture Center — The Ontology System](https://www.palantir.com/docs/foundry/architecture-center/ontology-system/)*

3×4 격자로 12칸이 나온다. 이게 온톨로지의 전체 설계도다. **"온톨로지"라는 한 단어가 실제로는 12개의 구체적인 구현 과제**라는 뜻이기도 하다.

**핵심은 Action이다.** 여기가 팔란티어가 BI 도구와 갈라지는 지점이다. 대시보드는 보여주고 끝난다. 온톨로지는 **원래 시스템에 되돌려 쓴다.**

![온톨로지가 read-write 루프를 가능하게 하는 다이어그램](https://www.palantir.com/docs/resources/foundry/architecture-center/ontology-read-write-loops.png)
*출처: [Palantir Architecture Center — The Ontology System](https://www.palantir.com/docs/foundry/architecture-center/ontology-system/)*

**이 그림이 이번에 읽은 것 중 가장 중요하다.** 화살표가 한 방향이 아니라 **고리(loop)** 로 돌아온다. 소스에서 읽어와 분석하고 끝나는 게 아니라, 결과가 다시 원래 시스템으로 흘러 들어간다.

"민원이 몰리고 있습니다"를 보여주는 것과 "긴급출동을 지시했습니다"를 실제 시스템에 기록하는 것은 다른 물건이다. 지금 우리 시스템에는 이 고리의 **왼쪽 절반밖에 없다.**
...
![다양한 데이터 소스·컴퓨트 엔진·모델을 보여주는 'Any Data, Any compute, Any model' 그림](https://www.palantir.com/docs/resources/foundry/architecture-center/mmdp-any.png)
*출처: [Palantir Architecture Center — Multimodal Data Plane](https://www.palantir.com/docs/foundry/architecture-center/multimodal-data-plane/)*
...
![MMDP가 온톨로지 계층과 자동화·에이전트·앱 계층을 떠받치는 그림](https://www.palantir.com/docs/resources/foundry/architecture-center/mmdp-layers.png)
*출처: [Palantir Architecture Center — Multimodal Data Plane](https://www.palantir.com/docs/foundry/architecture-center/multimodal-data-plane/)*

MMDP는 온톨로지 **아래**에 깔린다. 개방형 데이터·컴퓨트가 먼저 있고 그 위에 온톨로지가 선다는 순서다.
...
![팔란티어 플랫폼 상호운용성 그림](https://www.palantir.com/docs/resources/foundry/architecture-center/interoperability-layers.png)
*출처: [Palantir Architecture Center — Interoperability](https://www.palantir.com/docs/foundry/architecture-center/interoperability/)*
...
![Rubix — 강화되고 오토스케일링되는 고가용성 쿠버네티스 구현](https://www.palantir.com/docs/resources/foundry/architecture-center/rubix-k8s.png)
*출처: [Palantir Architecture Center — The Rubix substrate](https://www.palantir.com/docs/foundry/architecture-center/rubix/)*

핵심 설계 세 가지.

**1) 격리와 암호화**

![Rubix 보안 — 핵심 인프라 보안 기능 목록](https://www.palantir.com/docs/resources/foundry/architecture-center/rubix-security.png)
*출처: [Palantir Architecture Center — The Rubix substrate](https://www.palantir.com/docs/foundry/architecture-center/rubix/)*
...
![AIP 아키텍처 다이어그램 — 12개 역량 카테고리](https://www.palantir.com/docs/resources/foundry/architecture-center/aip-architecture.png)
*출처: [Palantir Architecture Center — AIP architecture](https://www.palantir.com/docs/foundry/architecture-center/aip-architecture/)*
