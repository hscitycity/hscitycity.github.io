...
Phase 3에서 Naver Cloud로 옮기고 PostgreSQL/PostGIS를 직접 운영하기 시작하면 얘기가 완전히 달라진다. 공간 DB에 화성시 도시 데이터가 쌓이는데, 그 DB를 인터넷에 그대로 열어둘 수는 없다. 그렇다고 아무도 못 들어가면 관리자가 접속해서 점검도 못 한다. 이 모순을 푸는 표준 답이 VPC, 공인/사설 IP, Subnet, NAT Gateway, 중계서버, DMZ, VPN이다.

이 글은 이 일곱 개념을 그림으로 정리한다. 처음 보면 낯선 용어들이라, 이해를 돕기 위해 "아파트 단지" 하나에 전부 빗대어 설명을 붙였다.
...
> 🏢 **비유**: VPC는 "내가 통째로 분양받은 아파트 단지 부지"다. 담장(경계) 안쪽은 전부 내 마음대로 설계할 수 있고, 옆 단지(다른 고객의 VPC)와는 완전히 분리되어 있다.
...
Subnet을 Public/Private으로 나누는 기준 자체가 "그 안의 자원이 공인 IP를 가질 수 있는가"다. 그래서 Subnet을 이해하기 전에 IP 두 종류의 차이부터 짚어야 한다.

> 🏢 **비유**: 공인 IP는 "도로명 주소"다. 택배 기사가 내비게이션에 찍으면 바로 찾아온다. 사설 IP는 "동-호수"다. 단지 안에서만 통하는 표시라서, 외부인은 이 번호만으로는 절대 못 찾아온다.

![Private IP Addresses](https://media.geeksforgeeks.org/wp-content/uploads/20260110120935313726/go.webp)

![Public IP Addresses](https://media.geeksforgeeks.org/wp-content/uploads/20260110121013231741/hnipo.webp)

- **공인 IP(Public IP)**: 인터넷 전체에서 유일한 주소. ISP나 클라우드 사업자가 할당하고, 전 세계 어디서나 그 주소로 직접 접속을 시도할 수 있다.
- **사설 IP(Private IP)**: 특정 사설망 안에서만 유효한 주소. 인터넷 라우터는 아래 대역(RFC 1918)을 목적지로 하는 패킷을 아예 전달하지 않는다 — 인터넷 입장에서는 "존재하지 않는" 주소다.

```
사설 IP 전용 대역 (RFC 1918)
...
| 구분 | 공인 IP | 사설 IP |
|------|---------|---------|
| 유일성 | 전 세계 유일 | 같은 사설망 안에서만 유일 |
| 할당 주체 | ISP/클라우드 (IANA 관리) | 네트워크 관리자가 자유롭게 부여 |
| 인터넷에서 직접 접속 | 가능 | 불가능 (라우터가 폐기) |
| 화성형 팔란티어 예 | Load Balancer, Bastion, NAT GW | K8s 워커 노드, PostGIS |

Private Subnet에 있는 K8s 워커 노드나 PostGIS는 처음부터 공인 IP 자체를 발급받지 않는다. 그래서 4번 문단의 NAT Gateway를 거칠 때, 자기 사설 IP 대신 NAT Gateway의 공인 IP를 "빌려서" 나가는 것이다.
...
> 🏢 **비유**: Public Subnet은 "단지 정문 앞 상가 구역"이다. 외부인도 자유롭게 드나든다. Private Subnet은 "안쪽 주거동"이다. 정문으로 나가는 도로 자체가 연결되어 있지 않아서, 외부인이 걸어서 찾아올 방법이 없다.
...
## 4. NAT Gateway: Private Subnet도 인터넷은 써야 한다
...
> 🏢 **비유**: 라우터는 "단지 안 교차로 표지판"이다. 트래픽이 어느 방향으로 가야 할지 안내만 할 뿐, 통과를 막거나 허용하지는 않는다. Internet Gateway는 "정문 하이패스 톨게이트"다. 등록된 차량(공인 IP를 가진 자원)은 양방향으로 자유롭게 드나든다. NAT Gateway는 "관리사무소의 택배 대리수령 창구"다. 안쪽 동 주민(Private Subnet)이 택배를 보낼 땐 문제없지만, 받을 땐 반드시 관리사무소 이름으로 먼저 받은 뒤 대신 전달해줘야 한다 — 외부에서 먼저 동/호수를 찍어 배송을 시작할 수는 없다.
...
## 5. 중계서버 (Bastion Host / Jump Server)

NAT Gateway가 "나가는 트래픽"을 위한 장치라면, Bastion Host는 "관리자가 들어오는 트래픽"을 위한 단일 관문이다.

> 🏢 **비유**: 중계서버(Bastion)는 "단지 경비실"이다. 방문객은 누구든 경비실에서 신원 확인을 받고 방문증을 발급받아야만 안쪽 동으로 들어갈 수 있고, 출입 기록은 전부 경비실 로그에 남는다.
...
> 🏢 **비유**: DMZ는 "단지 정문 앞 커뮤니티 센터"다. 외부인도 들어올 수 있는 개방된 공간이지만, 그 안쪽 진짜 주거 구역으로 가려면 카드키(내부 방화벽)를 한 번 더 통과해야 한다.
...
> 🏢 **비유**: Site-to-Site VPN은 "본사 건물과 지사 건물을 잇는 전용 지하 통로"다. 두 건물이 상시 연결되어 있다. Remote Access VPN은 "개인에게 발급된 원격 출입카드"다. 담당자가 재택근무 중에도 그 카드 하나로 특정 구역에 들어올 수 있다.
...
| 개념 | 한 줄 요약 | 아파트 단지 비유 |
|------|-----------|-----------------|
| VPC | 클라우드 안에 격리된 내 전용 네트워크 | 내가 통째로 분양받은 단지 부지 |
| 공인 IP / 사설 IP | 인터넷에서 직접 도달 가능한 주소 vs 사설망 안에서만 유효한 주소 | 도로명 주소 vs 동-호수 |
| Public/Private Subnet | "외부 노출 허용 여부"로 나눈 네트워크 구역 | 정문 앞 상가 vs 안쪽 주거동 |
| Router / Internet Gateway | 경로 안내 vs 인터넷과의 양방향 출입구 | 교차로 표지판 / 정문 하이패스 톨게이트 |
| NAT Gateway | Private Subnet의 아웃바운드 인터넷 접근을 대신 처리 | 관리사무소의 택배 대리수령 창구 |
| 중계서버(Bastion) | 관리자 접속을 한 곳으로 모아 감사·통제하는 관문 | 단지 경비실 |
| DMZ | 외부 노출 서비스와 내부망 사이의 완충 구역 | 정문 앞 커뮤니티 센터 |
| VPN | 인터넷 위에 뚫는 암호화 터널 (네트워크 간 / 개인 접속) | 본사-지사 전용 지하통로 / 개인 원격 출입카드 |

Phase 1에서는 이 일곱 개념 전부가 필요 없었다. Vercel이 다 가려줬기 때문이다. Phase 3에서 Naver Cloud로 옮기는 순간부터는 이 그림을 직접 설계해야 하고, 그 설계가 곧 화성형 팔란티어의 보안 수준을 결정한다.
