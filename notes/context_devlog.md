...
이 파일을 읽고 작업하는 너는 **2번 역할, DevLog 전용 에이전트**다.

### 절대 금지

> ### **`CONTEXT.md`를 절대 수정하지 마라.**

- 대상 파일: 프로젝트 루트의 `CONTEXT.md`
  (`…\hs_palantir_dataplatform\CONTEXT.md`)
- **읽는 것도 하지 않는다.** 수정·생성·이동·삭제는 물론 금지
- 사용자가 명시적으로 요청하지 않는 한, 어떤 이유로도 손대지 않는다
- 그 파일이 실제와 다르거나 낡아 보여도 **고치지 말고 사용자에게 보고만 한다**

**왜인가**: `CONTEXT.md`는 **1번 역할(Palantir-Demo 개발담당)의 소유**다. 인프라 구축 현황·K8s 매니페스트·배포 상태를 관리하는 파일이라, DevLog 담당이 건드리면 두 역할의 기록이 충돌하고 어느 쪽이 최신인지 알 수 없게 된다.

### DevLog 에이전트가 관리하는 것

| 구분 | 파일/경로 | 권한 |
|------|----------|------|
| ✅ 이 파일 | `notes/context_devlog.md` | 읽기·쓰기 |
| ✅ 블로그 포스트 | `blog/*.md` | 읽기·쓰기 |
| ✅ 본문 삽화 | `img/dev_img/` | 읽기·쓰기 |
| ✅ 작업 노트 | `notes/CHANGELOG.md` | 읽기·쓰기 |
| ✅ 블로그 저장소 전반 | `dev-log-blog/` 이하 | 읽기·쓰기 |
| ❌ **`CONTEXT.md`** | 프로젝트 루트 | **손대지 말 것** |
| ⚠️ `Palantir-Demo/` | 시스템 소스코드 | 1번 역할 소유. 참고만 |

**요약**: DevLog 에이전트의 작업 범위는 **`dev-log-blog/` 안쪽**이다. 그 바깥은 건드리지 않는다.
...
### 디자인 시스템 문서 → `notes/design.md`

블로그·대시보드 공유 시각 언어(Palantir HUD: 다크 네이비 + 사이언 글로우 + 그라디언트 타이포). 색 토큰·폰트·히어로/네비/카드 패턴·복붙 스니펫·접근성 규칙 정리. **새 화면 디자인 시 먼저 이 문서를 볼 것.**

### 헤더 최종 형태 (2026-07-25) — 대시보드 스타일 HUD 히어로

블로그 상단(`index.html`)을 대시보드 히어로와 동일한 룩으로 재구성(그 이전의 top-bar/오버레이/sticky 시도는 모두 이 형태로 대체됨):
- `.blog-hero`: 다크 이미지 풀블리드 + 라디얼 글로우 + 격자 + 하단 사이언 라인
- HUD 이어브로우 배지 + 그라디언트 대형 제목(`.hero-h1`, Paperlogy) + 서브타이틀
- 상단 네비(`.bh-nav`): 소개/개발일지/검색/대시보드/햄버거, 사이언 톤. 방문자 통계는 좌측
- **히어로는 sticky 아님**(스크롤 시 올라감) — 필요 시 슬림 고정바 추가 여지
- `js/header.js`: data-nav 위임·햄버거·검색·진행선(rAF)·FAB·현재페이지(`.bh-nav .hdr-link`)

---

### (구) 헤더 레이아웃 재배치 (2026-07-25, 겹침 해소)

방문자 통계바와 메뉴가 겹쳐서 재배치:
- **상단 유틸리티 바**(`.top-bar`, sticky): 좌=방문자 통계(`#visitor-stats`, fixed→static), 우=햄버거(`#hamburger-btn`). 하단에 읽기 진행선(`#hdr-progress`)
- **햄버거 드롭다운**(`#menu-drawer`): 항상 표시되는 메뉴 토글(소개/개발일지/대시보드/검색). 우상단 fixed
- **메뉴바는 이미지 하단 오버레이**(`.hero-menubar`, absolute bottom): 밝은 글자 + `.hero-banner::after` 스크림. 소개/개발일지/검색/구분선/대시보드
- **검색 패널**: 우상단 fixed 드롭다운(드로어와 상호배타)
- **글자 +2pt**: 링크 14→16, 대시보드 13→15, 워드마크 19→21
- **[data-nav] 위임**: 오버레이·드로어 링크 모두 `handleNav()`가 처리(중복 id 회피). `js/header.js` 재작성 — 햄버거/검색 토글, 진행선(rAF), 바깥클릭/Escape 닫기, 현재페이지 표시. 축소(shrink) 동작은 이번 재배치로 제거
- 색은 CSS 변수: 라이트 `--hdr-*` + 이미지 오버레이용 `--ov-*`(밝은색)

---

### (구) 상단 헤더 전면 재설계 (2026-07-25, 명세 기반) — 위 재배치로 일부 대체됨
...
- **개발관리 대시보드**: `menu/`의 .md가 아니라 저장소 루트의 **정적 `dashboard.html`**. `render.js`에서 메뉴 루프 뒤에 하드코딩으로 `<a href="dashboard.html" data-external="true">`를 append. `data-external="true"`면 SPA 렌더링을 거치지 않고 **같은 탭 이동**(데스크톱은 기본 링크 동작, 모바일은 `mobileMenuToggle.js`에서 `window.location.href`로 이동)
  - `dashboard.html`: 실제 대시보드(2026-07-25, 약 104KB). 구성: 종합 전략 카드(이슈 트리) + 팔란티어 vs 화성형 비교표 + 당직플랫폼 플로우 + 칸반 보드(카드 12, 드래그) + 협업 네트워크 11 + 현안과제 21 + 할 일 리스트 50 + 정보시스템 DB연계. 드래그앤드롭·담당/우선순위 필터·전략 모달(stratData)·카드 상세 모달 JS 내장. 외부 CDN은 Pretendard 폰트뿐
  ...
  - ⚠️ **인코딩 주의**: 이 대시보드를 채팅에 첨부/붙여넣기로 주면 UTF-8→Latin1 mojibake로 깨져서 도착함(3회 발생). **사용자가 로컬 파일을 직접 저장**하는 방식이 정확함. HTML을 받으면 항상 `file`로 UTF-8 확인 + `화성형 팔란티어` 등 한글 grep + `Ã` mojibake 흔적 grep으로 검증할 것
  - 2026-07-25 최종본은 사용자가 로컬에 직접 저장한 파일을 검증만 하고 커밋(한글 정상·div 680/680·JS 문법 OK 확인)
  - "최종 업데이트" 및 due 날짜는 파일 값 그대로 — 갱신은 사용자 몫
...
**미사용 2종**: thumb10(신경망 — AI/온톨로지 주제에 적합), thumb14(정글 수달 — 버그 헌팅 주제에 적합)
...
## 5. 현재 블로그 포스트 목록 (2026-07-25 세션 종료 기준, 총 16편)
...
| 20260725 | 2026.7.25.(토) 기준 작업현황 및 작업계획 | project | thumb10.png |
| 20260725 | 화성특례시 당직 업무 통합플랫폼 개발기 | project | thumb14.png |

카테고리 현황: `project`(3), `architecture`(5), `backend`(3), `llm`(1), `css`(1), `network`(1)

**썸네일 사용 현황** (2026-07-25 기준) — **14종 전부 사용됨, 미사용 0**
- 중복 2회: `thumb6.jpg`(20260610 + 20260717 작업현황), `thumb4.webp`(20260630 + 20260717 DB버그)
- thumb10(신경망)=협업 네트워크 글, thumb14(정글 수달)=당직플랫폼 개발기(버그 헌팅 주제)
- 이제 모든 새 글은 기존 썸네일 재사용 또는 신규 이미지 추가 필요. 재사용 시 주제 맞는 것 우선
...
### 2026-07-17 (노트북) — 저장소 정상화 + 포스트 4편 작성

**요약**: 블로그 10편 → **14편**. ZIP 사본이던 노트북 폴더를 git clone본으로 교체해 push 가능 상태로 만들고, 이 문서를 저장소 안(`notes/`)으로 이동. 인프라 진행도 kubectl 연결 → 파드 배포까지 나감.
...
### ⚠️ 1번 역할(Palantir-Demo 개발담당)에게 넘길 사항

**DevLog 에이전트는 `CONTEXT.md`를 수정하지 않으므로**(0번 항목), 아래 내용은 1번 역할이 직접 반영해야 한다. 2026-07-17 기준 `CONTEXT.md`와 실제가 어긋난 것으로 보이는 지점:

| CONTEXT.md 기재 (추정) | 실제 (0717 확인) |
|----------------------|-----------------|
| 3-1 kubectl 설치 ⏳ 대기 | ✅ 완료 |
| 3-2 매니페스트 작성 ⏳ 대기 | ✅ 최소한 secret/deployment는 apply됨 (파드 동작 중) |
| 2-3 NKS 클러스터 생성 중 | ✅ 프로비저닝 완료 |
| `DB_HOST: pg-47dq25.vpc-**pub**-cdb-kr.ntruss.com` | ❌ **Public 도메인 — 인터넷 우회 경로.** `pg-47dq25.vpc-cdb-kr.ntruss.com`(Private)으로 수정해야 함 |

**특히 마지막 항목이 중요하다.** `CONTEXT.md`의 `secret.yaml` 예시에 Public 도메인이 박혀 있으면, 다음에 그 예시를 복사해 쓰는 순간 같은 버그가 재발한다. (20260717 「VPC 내부인데 인터넷을 돌아간 DB 접속 버그」 글 참고)

또 프로젝트 루트 `작업메모.txt`에 NCP IAM 키가 평문으로 있었으나 2026-07-17 확인 시 파일이 사라짐 — 정리된 것으로 보이나, 루트를 `git init`할 때 재점검 필요.
- 류승인 주무관 MCP 서버를 Claude API `mcp_servers` 파라미터로 연결
- 전명구 주무관 플랫폼 파일 수령 후 통합 계획 상세화

---

### 2026-07-25 (노트북) — 주간 작업현황 포스트 + Notion 연동

**작성·배포**: 20260725 「2026.7.25.(토) 기준 작업현황 및 작업계획」 (project, thumb10.png)

- 요일 확인: 2026-07-25는 **토요일** 맞음 (`date -d`로 검증)
- **Notion MCP로 소스 2건을 직접 읽어 근거로 삼음**:
  - 개발 기록 DB(`collection://ac328250-8155-8336-969d-07e03c60d7cc`) — 7/19~7/24 인프라/배포 작업 지속 확인
  - 우수사례 페이지(`26f2825081558264a1a081c1812e9930`) — 협업 네트워크가 4명 → 9개 기관/개인으로 확장된 것 반영
- 글 두 축: (1) 인프라 배포 진행상황, (2) 전국 협업 네트워크 확장(핵심)

**⚠️ 개인정보 처리 원칙 (사용자 지시, 앞으로도 준수)**

> **소속·이름·직급은 블로그 게시 가능. 그 외 개인정보는 절대 게시 금지.**

블로그에 넣지 않은 것:
- 담당자 연락처 — 전화번호, 이메일 (예: 행안부 AI정부실험실 담당자 연락처). Notion 원본엔 있으나 블로그엔 제외
- 개발 기록의 **야간 작업 시간대**(22:00~익일 오전 패턴) — 개인 생활정보에 가까워 제외
- 내부 페이지(회의록·계약서·주요연락 담당자)는 언급 안 함

넣은 것: 이름+소속+직급, 공개된 포트폴리오/서비스 URL(GitHub·웹앱 등 이미 공개된 것)만 선별.

- **협업 대상은 전부 AI 챔피언 네트워크로 이미 협업 관계가 맺어진 분들**(사용자 확인, 2026-07-25). 실명+소속+직급+공개 링크 게시 OK. 따라서 우수사례에 등장하는 공직자들은 익명화·링크 제거 불필요 — 단, 위의 "연락처/이메일 등 개인정보 제외" 원칙은 그대로 유지

- push 후 검증: `grep -E '[0-9]{2,3}-[0-9]{3,4}-[0-9]{4}|@mail\.go\.kr'`로 전화/이메일 유출 없음 확인하는 절차 권장

**진행 상황 갱신 (7/17 → 7/25)**: kubectl 연결·파드 배포·DB 경로 수정 완료. 다음은 Ingress → LB → HTTPS → DNS 전환.

**두 번째 포스트(같은 날)**: 20260725 「화성특례시 당직 업무 통합플랫폼 개발기」 (project, thumb14.png)

- 별도 POC 프로젝트 개발기. POC: `duty-management-platform.vercel.app`, GitHub: `hscitycity/Duty-Management-Platform`
- 원본 제목 「[개발기] … — …」에서 **대괄호 `[개발기]`는 파일명에서 제외**(정규식 구분자와 충돌). 파일명 제목은 「화성특례시 당직 업무 통합플랫폼 개발기」, `[개발기]` 느낌은 본문 첫 줄 blockquote로 살림
- 삽질 기록 8종(④-1~8)이 본론. 스택: 단일 HTML 웹앱 + Leaflet/VWorld + Claude API + Supabase + Vercel
- **화성형 팔란티어 본체 글들과 교차 참조**: 단일 HTML 선택, 브라우저 API 프록시, 자동 검사 등 같은 교훈이 반복됨을 본문에서 연결(단, 상세 URL 형식 불명이라 하이퍼링크 대신 「제목」으로만 언급 — placeholder 홈링크 절대 넣지 말 것)
- 개인정보 이슈 없음(공개 링크만)
