...
## 5. 현재 블로그 포스트 목록 (2026-07-25 세션 종료 기준, 총 15편)
...
카테고리 현황: `project`(2), `architecture`(5), `backend`(3), `llm`(1), `css`(1), `network`(1)

**썸네일 사용 현황** (2026-07-25 기준)
- 미사용: **thumb14**(정글 수달)만 남음
- 중복 2회: `thumb6.jpg`(20260610 + 20260717 작업현황), `thumb4.webp`(20260630 + 20260717 DB버그 — 둘 다 버그 글이라 의도적)
- thumb10(신경망) → 20260725 작업현황 글에 사용(협업 네트워크 주제)

※ 중복은 동작에 문제 없음. 다만 목록에서 같은 그림이 반복되므로, 새 글은 미사용분(thumb14) 우선 검토
